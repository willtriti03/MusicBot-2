...
- 의존성은 현재 Python 환경과 `voice-sidecar/`에 직접 설치합니다.
...
`ecosystem.config.js`는 시스템 Python을 사용하도록 설정되어 있으며, 기본 `VoiceTransport=dave-sidecar`를 전제로 합니다.

```javascript
interpreter: 'python3',
```

필요하면 `python3.13`처럼 명시적인 실행 파일로 바꾸고, Node 경로를 고정해야 한다면 `MUSICBOT_NODE_BIN` 환경 변수를 추가하세요.
